...
import DilcaDistance.LaplacianNoiseGenerator;
 ...
 * @version $Revision$
 */
public class ContingencyTablesDiffPriv extends ContingencyTables
  implements RevisionHandler {
	...
	/** The natural logarithm of 2 */
	  ...
	  /*
	  public double lapNoise(Random rand, double mu, double epsilon) {
		double noise = LaplacianNoiseGenerator.nextLaplacian(rand, mu, epsilon);
		return noise;
		  
	  }
	  */
	  
	  public static double mySymmetricalUncertaintyNoised(double matrix[][], double epsilon, int dimensionData, int classesY, int classesX, long rand) {
	...
	/*
		   ...
		   for (int i = 0; i < matrix[0].length; i++) {
		      ...
		      total += sumForColumn;
		   //   System.out.println("sumForColumn: "+sumForColumn);
		   //   System.out.println("colen: "+columnEntropy);
			}
		   ...
		   for (int i = 0; i < matrix.length; i++) {
		      ...
		      //System.out.println("xxxxxx"+rowEntropy);
		   }
		   entropyConditionedOnRows -= rowEntropy;
		   rowEntropy -= lnFunc(total);
		   //infoGain = columnEntropy - entropyConditionedOnRows;
		   //System.out.println("yyy"+rowEntropy);
		   
	*/
		  columnEntropy = entropyOverColumns(matrix);
		  rowEntropy = entropyOverRows(matrix);
		  entropyConditionedOnRows = entropyConditionedOnRows(matrix);
		   
		   double sensitivityY = (2.0/dimensionData) * (log2(((dimensionData*classesY)/2.0)));
		   double sensitivityX = (2.0/dimensionData) * (log2(((dimensionData*classesX)/2.0)));
		   double sensitivityJoint = (2.0/dimensionData) * (log2(((dimensionData*(classesX*classesY))/2.0)));
		   
		   /* ENTROPIA H(Y) */
		   LaplacianNoiseGenerator noiseY = new LaplacianNoiseGenerator(rand, 0, (sensitivityY/epsilon));
		   double noiseColumn = noiseY.nextLaplacian();
		   double columnEntropyNoised = columnEntropy + noiseColumn;
		   
		   /* ENTROPIA H(X) */
		   LaplacianNoiseGenerator noiseX = new LaplacianNoiseGenerator(rand, 0, (sensitivityX/epsilon));
		   double noiseRow = noiseX.nextLaplacian();
		   double rowEntropyNoised = rowEntropy + noiseRow;
		   
		   /* ENTROPIA H(Y|X) */
		   LaplacianNoiseGenerator noiseJointed = new LaplacianNoiseGenerator(rand, 0, (sensitivityJoint/epsilon));
		   double noiseJoint = noiseJointed.nextLaplacian();
		   double entropyjoint = rowEntropy + entropyConditionedOnRows;  //entropia congiunta è possibile calcolarla come H(X) + H(Y|X)
		   double entropyjointNoised = entropyjoint + noiseJoint;
		   
		   /* INFOGAIN */
		   infoGain = columnEntropyNoised + rowEntropyNoised - entropyjointNoised;  //  H(Y) + H(X) - H(X,Y) = H(Y) + H(X) - H(X) - H(Y|X) = H(Y) - H(Y|X)
		   //double entropyConditionedOnRowsNoised = entropyConditionedOnRows + noiseY.nextLaplacian();
		   //infoGain = columnEntropyNoised - entropyConditionedOnRowsNoised; 
/*
		   System.out.println("rumore applicato alle colonne: "+ noiseColumn);
		   //System.out.println("entropia colonna SENZA rumore: "+ columnEntropy);
		   System.out.println("entropia colonna con rumore: "+ columnEntropyNoised);
		   System.out.println("entropia colonna senza rumore: "+ columnEntropy);
		   System.out.println("rumore applicato alle righe: "+  noiseRow);
		   //System.out.println("entropia righe SENZA rumore: "+ rowEntropy);
		   System.out.println("entropia righe con rumore: "+ rowEntropyNoised);
		   System.out.println("entropia righe senza rumore: "+ rowEntropy);
		   System.out.println("rumore applicato all'entropia congiunta: "+ noiseJoint);
		   System.out.println("sensitivity Y: "+ sensitivityY);
		   System.out.println("mu Y: "+ noiseY.getMu());
		   System.out.println("b Y: "+ noiseY.getB());
		   */
		   System.out.println("SU: "+ 2.0 * (infoGain / (columnEntropyNoised + rowEntropyNoised)));
		   System.out.println("******************");
		   
		   //System.out.println(epsilon);
		   if (Utils.eq(columnEntropyNoised, 0) || Utils.eq(rowEntropyNoised, 0))
		     return 0;
		   return 2.0 * (infoGain / (columnEntropyNoised + rowEntropyNoised));
		 }
	  ...
	  public static double symmetricalUncertainty(double matrix[][]) {
	    ...
	    double sumForColumn, sumForRow, total = 0, columnEntropy = 0, 
	      rowEntropy = 0, entropyConditionedOnRows = 0, infoGain = 0, columentrprova =0, rowentrprov = 0, conditientrprova = 0;
	  ...
	  /*  for (int i = 0; i < matrix.length -1; i++) {
	    ...
	    */
	    ...
	    for (int i = 0; i < matrix[0].length; i++) {
	      ...
	      columnEntropy += lnFuncMod(sumForColumn);
	      total += sumForColumn;
	   //   System.out.println("sumForColumn: "+sumForColumn);
	   //   System.out.println("colen: "+columnEntropy);
	    }
	    columnEntropy -= lnFuncMod(total);
		...
		entropyConditionedOnRows += lnFunc(matrix[i][j]);
	      ...
	      rowEntropy += lnFuncMod(sumForRow);
	      //System.out.println("xxxxxx"+rowEntropy);
	    }
	    entropyConditionedOnRows -= rowEntropy;
	    rowEntropy -= lnFuncMod(total);
	    //System.out.println("yyy"+rowEntropy);
	    infoGain = columnEntropy - entropyConditionedOnRows;
	    if (Utils.eq(columnEntropy, 0) || Utils.eq(rowEntropy, 0))
	      return 0;
	    
	    columentrprova = entropyOverColumns(matrix);
	    rowentrprov = entropyOverRows(matrix);
	    conditientrprova = entropyConditionedOnRows(matrix);
	     //SIMILARITA' TRA I MODI DI CALCOLARE LE ENTROPIE
	    /*
	    System.out.println("entropy column originale: "+ columnEntropy);
	    System.out.println("entropy column versione rivista: "+ columentrprova);
	    System.out.println("SU versione originale: "+ 2.0 * (infoGain / (columnEntropy + rowEntropy)));
	    System.out.println("SU versione rivista: "+ 2.0 * ((columentrprova-conditientrprova) / (columentrprova + rowentrprov)));
	    */
	    //return 2.0 * (infoGain / (columnEntropy + rowEntropy));
	    return 2.0 * ((columentrprova-conditientrprova) / (columentrprova + rowentrprov));
	  }
	  
	  
	  /**
	   * Calculates the symmetrical uncertainty noised with Laplace mechanism for base 2.
	   *
	   * @param matrix the contingency table
	   * @return the calculated symmetrical uncertainty
	   *
	   */
	// public static double symmetricalUncertaintyNoised(double matrix[][], double epsilon) {
	//
	//   double sumForColumn, sumForRow, total = 0, columnEntropy = 0, 
//	     rowEntropy = 0, entropyConditionedOnRows = 0, infoGain = 0;
	//
...
//	     rowEntropy += -(lnFunc(sumForRow));
	//   }
	//   double dimensionData = matrix.length;
	//   double sensitivity = (2.0/4) * (Math.log((4/2.0)));
	//   //double sensitivity = 1.5;
	//   LaplaceDistribution noise = new LaplaceDistribution(0,(sensitivity/epsilon));
	//   double noiseColumn = noise.density(columnEntropy);
	//   System.out.println("column entropy: " + columnEntropy);
	//   double columnEntropyNoised = columnEntropy + noiseColumn;
	//   entropyConditionedOnRows -= rowEntropy;
	//   double entropyConditionedOnRowsNoised = entropyConditionedOnRows + noise.density(entropyConditionedOnRows);
	//   rowEntropy -= -(lnFunc(total));
	//   double rowEntropyNoised = rowEntropy + noise.density(rowEntropy);
	//   infoGain = columnEntropyNoised - entropyConditionedOnRowsNoised;
	//   System.out.println("rumore applicato alle colonne: "+ noiseColumn);
	////   System.out.println("rumore applicato alle righe: "+ noise.density(rowEntropy));
	////   System.out.println("rumore applicato alle y dato x: "+ noise.density(entropyConditionedOnRows));
	//   //System.out.println("epsilon: "+ noise.getScale());
	//   //System.out.println("cumulativeProbability: "+ noise.cumulativeProbability(columnEntropy));
	//
	//   //System.out.println("y distorta: "+ columnEntropyNoised);
	//   System.out.println("H(Y) noise: "+ columnEntropyNoised);
	//   System.out.println("H(Y|X) noise: "+ entropyConditionedOnRowsNoised);
	//   System.out.println("H(X) noise: "+ rowEntropyNoised);
	//   System.out.println("sensitivitÃ : "+sensitivity + ", dimensione dataset" + dimensionData);
	//   if (Utils.eq(columnEntropyNoised, 0) || Utils.eq(rowEntropyNoised, 0))
//	     return 0;
	//   return 2.0 * (infoGain / (columnEntropyNoised + rowEntropyNoised));
	// }
	  ...
	  public static double lnFuncMod(double num){
...
//	    System.out.println("Symmetrical uncertainty NOISED: " +
//			       ContingencyTables.symmetricalUncertaintyNoised(matrix, 0.1));

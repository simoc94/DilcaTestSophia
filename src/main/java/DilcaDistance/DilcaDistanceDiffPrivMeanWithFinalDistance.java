...
public class DilcaDistanceDiffPrivMeanWithFinalDistance extends DilcaDistance implements DistanceFunction, Serializable {
	...
	public double getEpsilonContext() {
		return epsilonContext;
	}

	public void setEpsilonContext(double epsilonContext) {
		this.epsilonContext = epsilonContext;
	}

	public DilcaDistanceDiffPrivMeanWithFinalDistance(){
	...
	public DilcaDistanceDiffPrivMeanWithFinalDistance(double epsilonContext, double epsilonDistance, double sigma){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
	...
	public DilcaDistanceDiffPrivMeanWithFinalDistance(double epsilonContext, double epsilonDistance, double sigma, long rand){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
...
//		System.out.println();			
	...
	private double[][] matrixBetAttrNoised(Instances data, int indexTarget, int indexAttrCont){
		...
		//double [] normalization = new double[data.attribute(indexAttrCont).numValues()];
		for (int i=0;i<data.numInstances();i++){
			...
			//normalization[secondValue]++;
		}
		
		
		double sensitivity = (2.0)/(epsilonDistance);
		System.out.println("sensitivity: "+sensitivity);
		LaplacianNoiseGenerator noise = new LaplacianNoiseGenerator(1478698520, 0, (sensitivity));
...
//			System.out.println(reducedData);
		...
		int numValuesF = reducedData.attribute(indexF).numValues(); 
		//System.out.println("num values: "+numValuesF); 	//totale valori attributo target
		//System.out.println("totNumFeature: "+totNumFeature); //totale valori degli attributi del context
		double [] normalization = new double[totNumFeature];
		double[][]matrixComputation = new double[numValuesF][];
		for(int j =0; j<nearest.size();j++){
			...
			if(indexF<nearest.get(j)) {
				 matr = matrixBetAttrNoised(reducedData, indexF, nearest.get(j));
				 //System.out.println("NOT traspose");
			}else {
				matr = transpose(matrixBetAttrNoised(reducedData,nearest.get(j),indexF));
				//System.out.println("num righe "+matr.length);
				//System.out.println("traspose");
			}
			concate(matrixComputation, matr);
		...
		for (int i=0; i< n_attributes; i++){
			try{
				...
				if (data.attribute(i).numValues() > 1){
					...
					SymmetricalUncertAttributeSetEvalDiffPriv eval = new SymmetricalUncertAttributeSetEvalDiffPriv(epsilonContext, rand);

...
public class DilcaDistanceDiffPrivRR extends DilcaDistance implements DistanceFunction, Serializable {
	...
	public DilcaDistanceDiffPrivRR(){
	...
	public DilcaDistanceDiffPrivRR(double epsilon){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
	...
	public DilcaDistanceDiffPrivRR(double epsilon, long rand){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
		...
		for (int i=0; i< n_attributes; i++){
			try{
				...
				if (data.attribute(i).numValues() > 1){
					...
					SymmetricalUncertAttributeSetEvalDiffPriv eval = new SymmetricalUncertAttributeSetEvalDiffPriv(epsilon, rand);
					...
					*/
					...
					/*
					FileWriter writer = new FileWriter("C:\\Users\\Simone\\Desktop\\TestSU\\TitanicSUDistorta.txt", true);
					if(i==0 && indices[0]==5) {
						writer.write("Su DilcaRR: " + rankAtt[0][1]);
					}
					writer.close();
					 */
...
//			System.out.println(reducedData);
	...
	//e' il contrario, indexTarget è quello del context e indexAttrCont è target
	public double SU(Instances data, int indexTarget, int indexAttrCont){
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		double[][] matrix = new double[reducedData.attribute(indexTarget).numValues()][reducedData.attribute(indexAttrCont).numValues()];
		...
		for (int i=0;i<reducedData.numInstances();i++){
			int firstValue= (int) reducedData.instance(i).value(indexTarget);
			int secondValue = (int) reducedData.instance(i).value(indexAttrCont);
		...
		*/
		//System.out.println(ContingencyTablesDiffPriv.mySymmetricalUncertaintyNoised(matrix,epsilon, reducedData.numInstances(), reducedData.attribute(indexTarget).numValues(),reducedData.attribute(indexAttrCont).numValues(), rand));
		return ContingencyTablesDiffPriv.mySymmetricalUncertaintyNoised(matrix,epsilon, reducedData.numInstances(), reducedData.attribute(indexTarget).numValues(),reducedData.attribute(indexAttrCont).numValues(), rand);
		
	}
	
	
	public static double SUvalue(Instances data) throws Exception {
		...
		double value = 0;
				...
				if (data.attribute(0).numValues() > 1){
					...
					rankAtt = attSel.rankedAttributes();
					System.out.println("rankatt: ");
					for (double [] row: rankAtt) {
				    	System.out.println(Arrays.toString(row));
				    }

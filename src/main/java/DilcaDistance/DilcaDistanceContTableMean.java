...
import DilcaDistance.SymmetricalUncertAttributeSetEvalContTable;
...
public class DilcaDistanceContTableMean implements DistanceFunction, Serializable{
	...
	public DilcaDistanceContTableMean(){
	...
	public DilcaDistanceContTableMean(double epsilon, double sigma){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
	...
	public DilcaDistanceContTableMean(double epsilon, double sigma, long rand){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
...
*/
	...
	protected void computeMatrix() throws Exception{
		matricesDilca = new Vector<double[][]>(); 
...
//		System.out.println();			
	...
	private double [][] calculateFeatureDistance(Instances data, int indexF,Vector<Integer> nearest) throws Exception{
		...
		int numValuesF = data.attribute(indexF).numValues(); //numValues restituisce il num di valori dell'attributo e restituisce 0 per gli attributi che non sono nominali , stringhe o relation-valued
		...
		for(int j =0; j<nearest.size();j++){
			...
			matr = SymmetricalUncertAttributeSetEvalContTable.matrContingency(indexF, nearest.get(j), epsilon);
	...
	public double [][] calculateDistanceAttr(Instances data, int indexF,Vector<Integer> nearest) throws Exception{
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		//vector that contains in each position the sum of the number of the previous attribute values
				...
				//to manage the case of unary attribute
				if (nearest.size() > 0){
					totNumFeature = reducedData.attribute(nearest.get(0)).numValues();
					featValNum[0]=0;
				}
				
				for (Integer i=0;i<nearest.size()-1;i++){
					featValNum[i+1] = featValNum[i] + reducedData.attribute(nearest.get(i)).numValues(); 
					totNumFeature+=reducedData.attribute(nearest.get(i+1)).numValues();;
				}
				int numValuesF = reducedData.attribute(indexF).numValues(); //numValues restituisce il num di valori dell'attributo e restituisce 0 per gli attributi che non sono nominali , stringhe o relation-valued
				
				double [] normalization = new double[totNumFeature];
				double[][]matrixComputation = new double[numValuesF][];
				for(int j =0; j<nearest.size();j++){
					double[][] matr = new double[numValuesF][reducedData.attribute(nearest.get(j)).numValues()];	
					matr = SymmetricalUncertAttributeSetEvalContTable.matrContingency(indexF, nearest.get(j), epsilon);
					concate(matrixComputation, matr);
				}
				/*
		...
		for (int i=0; i< n_attributes; i++){
			try{
				...
				if (data.attribute(i).numValues() > 1){
					...
					SymmetricalUncertAttributeSetEvalContTable eval = new SymmetricalUncertAttributeSetEvalContTable(epsilon, rand);
					MeanSearch search = new MeanSearch(sigma);
	 ...
	 * @throws Exception 
	 */
	public int[] context(Instances data, int attributeTarget) {
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		int n_attributes = reducedData.numAttributes();
		System.out.println("num attributes: "+n_attributes);
		//Vector<Vector<Integer>> nearIndex = new Vector<Vector<Integer>>(n_attributes);
		int originalClass = reducedData.classIndex(); 
		AttributeSelectionMean attSel = new AttributeSelectionMean();
		int[] actual = null;
		try{
		if (reducedData.attribute(attributeTarget).numValues() > 1){
			reducedData.setClassIndex(attributeTarget);
			SymmetricalUncertAttributeSetEvalContTable eval = new SymmetricalUncertAttributeSetEvalContTable(epsilon, rand);
			MeanSearch search = new MeanSearch(sigma);
				attSel.setEvaluator(eval);
				attSel.setSearch(search);
				attSel.SelectAttributes(reducedData);
			...
			*/
				...
				actual = new int[indices.length-1];
				
				for (int j=0;j<indices.length;++j) {
					if ( indices[j] != attributeTarget ) 
						actual[j] = (indices[j]);
				}
			...
			//nearIndex.add(actual);	
			//System.out.println("Attributo: "+attributeTarget+nearIndex);
		
			for(int i = 0; i< actual.length;i++ ) {
				System.out.println(actual[i]);
			}
		
		reducedData.setClassIndex(originalClass);
		return actual;
	}
	 ...
	 * @throws Exception 
	 */
	public Vector<Integer> contextVectorforDistance(Instances data, int attributeTarget) throws Exception {
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		*/
			...
			for (int j=0;j<indices.length;++j) {
				if ( indices[j] != attributeTarget) actual.add(indices[j]);
			}
		}	
			nearIndex.add(actual);	
			System.out.println("Attributo: "+attributeTarget+actual);

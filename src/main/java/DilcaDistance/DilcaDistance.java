		...
		for (int i=0; i< n_attributes; i++){
			try{
				...
				if (data.attribute(i).numValues() > 1){
					...
					*/
					/*
					FileWriter writer = new FileWriter("C:\\Users\\Simone\\Desktop\\TestSUSoybean\\SoybeanSU.txt", true);
					if(i==0 && indices[0]==27) {
						writer.write("Su DilcaOriginal: " + rankAtt[0][1]);
					}
					writer.close();
					*/
...
//			System.out.println(reducedData);
		...
		*/
		System.out.println(ContingencyTables.symmetricalUncertainty(matrix));
		
		return ContingencyTablesDiffPriv.symmetricalUncertainty(matrix);

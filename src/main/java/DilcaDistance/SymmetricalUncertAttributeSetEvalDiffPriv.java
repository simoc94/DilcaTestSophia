 ...
 * @see Discretize
 */
public class SymmetricalUncertAttributeSetEvalDiffPriv
  extends SymmetricalUncertAttributeSetEval
  ...
  private static Instances m_trainInstances;
  ...
  private static int m_numInstances;

  /** The number of classes */
  private static int m_numClasses;

  /** Treat missing values as a seperate value */
  private static boolean m_missing_merge;
   ...
   * Constructor
   */
  public SymmetricalUncertAttributeSetEvalDiffPriv (double epsilon) {
    resetOptions();
   ...
   * Constructor2
   */
  public SymmetricalUncertAttributeSetEvalDiffPriv () {
    resetOptions();
  }
   ...
   * Constructor3
   */
  public SymmetricalUncertAttributeSetEvalDiffPriv (double epsilon, long rand) {
    resetOptions();
    this.epsilon = epsilon;
    this.rand= rand;
  }
...
//    double[][] mvg = new double[ni][nj];
//    for(int p =0; p <ni; p++) {
//    	for(int q = 0; q<nj;q++) {
//    		mvg[p][q]= Math.sqrt(counts[p][q]);
//    	}
//    }
    
//    double[][] sumMx = new double[ni][nj];
//    Random rand = new Random(); 
//    for (int r = 0; r < ni; r++) {
//        for (int s = 0; s < nj; s++) {
//        	sumMx[r][s] = counts[r][s] + rand.nextGaussian();  //replace '+' with '-' to subtract matrices
//        }
//    }
    
//    System.out.println("matrice di contingenza: " + Arrays.deepToString(sumMx));
//    System.out.println("valore di SU: " + ContingencyTables.symmetricalUncertainty(sumMx));
    

/*    
    System.out.println("matrice di contingenza: " + Arrays.deepToString(counts));
    System.out.println("valore di SU: " + ContingencyTables.symmetricalUncertainty(counts));
    //System.out.println("valore di SU noise: " + ContingencyTables.mySymmetricalUncertaintyNoised(counts,0.1, m_numInstances, m_numClasses));
    //System.out.println("entropia su colonne" + ContingencyTables.entropyOverColumns(counts));
    //System.out.println("entropia su righe" + ContingencyTables.entropyOverRows(counts));
    System.out.println("numeor classi "+m_numClasses);
    //System.out.println("entropia singola colonna" + ContingencyTables.entropy();
    System.out.println("Entropia su colonne: " + ContingencyTables.entropyOverColumns(counts));
    System.out.println("Entropia su righe: " + ContingencyTables.entropyOverRows(counts)); */
    ...
    /*
    System.out.println("numero di valori classi X: "+ m_trainInstances.attribute(attribute).numValues());
    System.out.println("nome attributo X: "+ m_trainInstances.attribute(attribute).name());
    System.out.println("numero di valori classi Y: "+ m_numClasses);
    */
    
    
    
    //System.out.println("lunghezza count righe "+(counts.length-1) +"; lunghezza counts colonne "+(counts[0].length-1) );
    return  ContingencyTablesDiffPriv.mySymmetricalUncertaintyNoised(counts,epsilon, m_numInstances, m_numClasses,m_trainInstances.attribute(attribute).numValues(), rand);
  }
  /*
...
*/
  ...
  public static double matrContingency(int attributeTarget, int attrContext, double epsilon)
		    throws Exception {
		    int i, j, ii, jj;
		    int ni, nj;
		    double sum = 0.0;
		    ni = m_trainInstances.attribute(attributeTarget).numValues() + 1;
		    nj = m_trainInstances.attribute(attrContext).numValues() + 1;
		    double[] sumi, sumj;
		    Instance inst;
		    double temp = 0.0;
		    sumi = new double[ni];
		    ...
		    for (i = 0; i < m_numInstances; i++) {
		      ...
		      if (inst.isMissing(attributeTarget)) {
		        ii = ni - 1;
		      }
		      else {
		        ii = (int)inst.value(attributeTarget);
		      }

		      if (inst.isMissing(attrContext)) {
		        jj = nj - 1;
		      }
		      else {
		        jj = (int)inst.value(attrContext);
		      }
			...
			double[][] matr = new double[counts.length-1][counts[0].length-1];
			for (int p = 0; p<counts.length-1;p++) {
				for(int q = 0; q<counts[0].length-1;q++) {
					matr[p][q] = counts[p][q]; 
		 		}
			}
			
		    
		    return  ContingencyTablesDiffPriv.mySymmetricalUncertaintyNoised(matr,epsilon, m_numInstances, m_numClasses,m_trainInstances.attribute(attrContext).numValues(), rand);
		  }
  ...
  public double evaluateAttribute (int[] attributes, int[] classAttributes)
    ...
    /*
    //System.out.println("symmetric numero classi: "+m_numClasses);
    for(int z = 0; z < attributes.length;z++) {
    	System.out.println("attributes: "+z+"  "+attributes[z]);
    }
    for(int z = 0; z < classAttributes.length;z++) {
    	System.out.println("classAttributes: "+z+"  "+classAttributes[z]);
    }
    //System.out.println("symmetric numero classi attr X: "+rowValues);
    for (int y = 0; y < counts.length -1; y++) {
        for (int z = 0; z < counts[0].length -1; z++) {
          System.out.print(" " + counts[y][z]+ ", ");
        }
   }
   System.out.println();
   //System.out.println("lunghezza count righe "+(counts.length-1) +"; lunghezza counts colonne "+(counts[0].length-1) );
   
   System.out.println("numero valori classe y: "+(counts[0].length-1));
   System.out.println("numero valori classe x: "+(counts.length-1));
   */
    
    
    return  ContingencyTablesDiffPriv.mySymmetricalUncertaintyNoised(counts,epsilon, m_numInstances, (counts[0].length-1), (counts.length-1), rand);
    //return ContingencyTables.symmetricalUncertainty(counts);
  }
  ...
  public static void main (String[] argv) {
    runEvaluator(new SymmetricalUncertAttributeSetEvalDiffPriv(), argv);
  }
}

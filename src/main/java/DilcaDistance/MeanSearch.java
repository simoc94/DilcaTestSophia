...
public class MeanSearch 
   ...
   * Constructor
   */
  public MeanSearch (double sigma) {
    resetOptions();
    this.sigma = sigma;
  }
  ...
  public int[] search (ASEvaluation ASEval, Instances data)
   ...
   // UTILI A ME
    for (i = 0; i < m_attributeList.length; i++) {
    	System.out.println("m_attrbitutesMErit: "+m_attributeMerit[i]);
        
    }
   
    /* aggiungo rumore per evitare valori negativi
     * 
     */
    double min = m_attributeMerit[0];
    for(int s = 0; s<m_attributeMerit.length;s++) {
    	if(m_attributeMerit[s]<min) {
    		min = m_attributeMerit[s];
    	}
    }
    for(int s = 0; s<m_attributeMerit.length;s++) {
    	m_attributeMerit[s] = m_attributeMerit[s] + Math.abs(Math.min(min, 0));
    }
    /* fino a questo punto ha ottenuto i vari valori della SU   */
   ...
   // UTILI A ME
    for (i = 0; i < rankedAttributes.length; i++) {
    	System.out.println("ranked attributes MEAN: "+rankedAttributes[i]);
        
      }
    
    System.out.println("m_numToSelect"+m_numToSelect);
    System.out.println("m_calculatedNumToSelect"+m_calculatedNumToSelect);
  ...
  public double[][] rankedAttributes ()
    ...
    int[] ranked = Utils.sort(m_attributeMerit);   //li ordina dal valore piÃ¹ piccolo al piÃ¹ grande
    // reverse the order of the ranked indexes
    double[][] bestToWorst = new double[ranked.length][2];

    /* tramite bestToWorst li ordine dal migliore al peggiore, perchÃ© in ranked li ordina ma dal piÃ¹ piccolo al piÃ¹ grande (es 0, 0.5, -0.5 ranked li ordina come -0.5, 0, 0.5, invece con bestToWorst li ordina come 0.5, 0, -0.5) */
   ...
   /* 
    for (int number : ranked) {
        System.out.println("ranked = " + number);
     }
    */
    
    
    /* inserisce nella matrice bestToWorst il numero dell'attributo e il valore, quindi nella prima riga ci sarÃ  nella prima colonna il numero dell'attributo e nella seconda colonna il valore della SU, ordinati dalla SU piÃ¹ alta a quella piÃ¹ bassa  */
    ...
    /*	STAMPA PER FAR CAPIRE CIO CHE FA NEL FOR SOPRA
    for (double [] row: bestToWorst) {
  ...
  private void computeMean(double[][]rankedFeatures)throws Exception {
	  
	  int numSelectedAttributes = 0;
	  double somma = 0;
	  for (int z = 0; z < rankedFeatures.length; z++) {
		  somma = somma + rankedFeatures[z][1];
	  }
	  //System.out.println("denominatore media "+rankedFeatures.length);
	  double media = somma/rankedFeatures.length;
	  //System.out.println("media: "+media);
	  //System.out.println("bestToWorst.length: "+bestToWorst.length);
	  System.out.println("media*"+sigma+": "+(sigma*media));
	  for (int z = 0; z < rankedFeatures.length; z++) {
		  if(rankedFeatures[z][1] >= (sigma*media)) {
    		numSelectedAttributes++;
		  }
	  }
	  System.out.println("numSelectedAttributes: "+numSelectedAttributes);
	  m_selectedFeatures = new double[numSelectedAttributes][2];
    
	  for(int z = 0; z < m_selectedFeatures.length; z++) {
		  if(rankedFeatures[z][1] >= (sigma*media)) {
			  m_selectedFeatures[z][0] = rankedFeatures[z][0];
			  m_selectedFeatures[z][1] = rankedFeatures [z][1];
		  }
	  }
	/*    System.out.println("m_selectedFeatures");
	    for (double [] row: m_selectedFeatures) {
	    
	    	System.out.println(Arrays.toString(row));
	    }
	    System.out.println(" fine m_selectedFeatures");
	*/
  /*  int i,j;

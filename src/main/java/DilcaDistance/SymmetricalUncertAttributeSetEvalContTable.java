 ...
 * @see Discretize
 */
public class SymmetricalUncertAttributeSetEvalContTable
...
/**
   ...
   * Constructor
   */
  public SymmetricalUncertAttributeSetEvalContTable () {
    resetOptions();
  }
   ...
   * Constructor2
   */
  public SymmetricalUncertAttributeSetEvalContTable (double epsilon) {
    resetOptions();
   ...
   * Constructor3
   */
  public SymmetricalUncertAttributeSetEvalContTable (double epsilon, long rand) {
    resetOptions();
    this.setEpsilon(epsilon);
    this.setRand(rand);
  }
  ...
  public double evaluateAttribute (int attribute)
    ...
    /*	STAMPA MATRICE PRIMA DELL'APPILCAZIONE DELLA DP
    System.out.println("Matrice prima dell' applicazione diff privacy: ");
    ...
    */
	...
	for (int p = 0; p<counts.length-1;p++) {
		for(int q = 0; q<counts[0].length-1;q++) {
			...
			counts[p][q] = counts[p][q] + noise.nextLaplacian();
			if(counts[p][q]<0) {
				counts[p][q] = 0;
			}
			counts[p][q] = (int)Math.round(counts[p][q]);
			
 		}
	}
	//System.out.println();
	
	/*	STAMPA MATRICE DOPO APPLICAZIONE DP
	System.out.println("Matrice dopo applicazione diff privacy: ");
	for (int p = 0; p<counts.length-1;p++) {
		for(int q = 0; q<counts[0].length-1;q++) {
			System.out.print(" " + counts[p][q]+ ", ");
 		}
...
/*    
 ...
 /*   for (i = 0; i < ni - 1; i++) {
    ...
    //System.out.println("numero classi 222: "+ m_numClasses);
	
	System.out.println("Su: "+ContingencyTables.symmetricalUncertainty(counts));
    return  ContingencyTables.symmetricalUncertainty(counts);
    
    //return  ContingencyTables.mySymmetricalUncertaintyNoised(counts,0.5, m_numInstances, m_numClasses);
  }
  ...
  public double evaluateAttribute (int[] attributes, int[] classAttributes)
    ...
    /*
    System.out.println("Matriceeeeeeeeeeeeeeeeeeeeeee prima dell' applicazione diff privacy: ");
	for (int cc = 0; cc<counts.length-1;cc++) {
		for(int q = 0; q<counts[0].length-1;q++) {
			System.out.print(" " + counts[cc][q]+ ", ");
 		}
	}
	System.out.println();
    */
	double sensitivity = (2)/(epsilon);
	//System.out.println("sensitivity: "+sensitivity);
	LaplacianNoiseGenerator noise = new LaplacianNoiseGenerator(rand,0, (sensitivity));
	...
	for (int cc = 0; cc<counts.length-1;cc++) {
		for(int q = 0; q<counts[0].length-1;q++) {
			//System.out.print("noise di "+ counts[cc][q] +": "+noise.nextLaplacian() + ",  ");
			counts[cc][q] = counts[cc][q] + noise.nextLaplacian();
			if(counts[cc][q]<0) {
				counts[cc][q] = 0;
			}
			counts[cc][q] = (int)Math.round(counts[cc][q]);
			
 		}
	}
	//System.out.println();
	/*
	System.out.println("Matriceeeeeeeeeee dopo applicazione diff privacy: ");
	for (int cc = 0; cc<counts.length-1;cc++) {
		for(int q = 0; q<counts[0].length-1;q++) {
			System.out.print(" " + counts[cc][q]+ ", ");
 		}
    ...
    */
	...
	//System.out.println(ContingencyTables.symmetricalUncertainty(counts));
  ...
  public static void main (String[] argv) {
    runEvaluator(new SymmetricalUncertAttributeSetEvalContTable(), argv);
  }

...
public class DilcaDistanceMean implements DistanceFunction, Serializable{
	...
	public DilcaDistanceMean(double sigma){
		matricesDilca = new Vector<double[][]>();
		m_Disc = new weka.filters.unsupervised.attribute.Discretize();
		m_RepMissValue = new ReplaceMissingValues();
		supDiscr = false;
...
//		System.out.println();			
	...
	public double [][] calculateDistanceAttr(Instances data, int indexF,Vector<Integer> nearest){
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		for (int i=0;i<reducedData.numInstances();i++){
				int firstValue= (int) reducedData.instance(i).value(indexF);
			for (int j=0;j<nearest.size();j++){
				int secondValue = (int) reducedData.instance(i).value(nearest.get(j));
				matrixComputation[firstValue][secondValue+featValNum[j]]++;
				normalization[secondValue+featValNum[j]]++;
			}
		}
...
//			System.out.println(reducedData);
	...
	public Vector<Vector<Integer>> contextProva(Instances data) {
		m_Data = data;
		initializeAttributeIndices();
		try{ 
			m_RepMissValue.setInputFormat(m_Data);
...
//			System.out.println(reducedData);
		...
		for (int i=0; i< n_attributes; i++){
			try{
				...
				Vector<Integer> actual = new Vector<Integer>();
				if (reducedData.attribute(i).numValues() > 1){
					reducedData.setClassIndex(i);
					SymmetricalUncertAttributeSetEval eval = new SymmetricalUncertAttributeSetEval();
					MeanSearch search = new MeanSearch(sigma);
					attSel.setEvaluator(eval);
					attSel.setSearch(search);

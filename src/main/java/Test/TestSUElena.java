...
public class TestSUElena {
	public static void main(String[] args) throws Exception {
		...
		*/
        ...
        Random rand =new Random();
		double epsilon = 0.0;
		FileWriter writer = new FileWriter("C:\\Users\\Simone\\Desktop\\Mushroom2SUElena.txt", true);
			for(int ciclo = 1; ciclo<500; ciclo++) {
				rand.setSeed(11235813);
				epsilon = epsilon+0.005;
				double distanza = 0.0;
				for(int i = 0; i<100;i++) {
					...
					for(int cc = 0; cc<cpu.numAttributes();cc++) {
						if(attributo!= attributoTarget) {
							...
							if(dd.SU(cpu,attributo,attributoTarget)>1.0) {
								...
								SUDist = SUDist + dd.SU(cpu,attributo,attributoTarget);
							}
							SUOrig = SUOrig + de.SU(cpu, attributo,attributoTarget);
					...
					distanza = distanza + Math.abs((SUDist/(cpu.numAttributes()-1))-(SUOrig/(cpu.numAttributes()-1)));
				}
				double result = (double) distanza/100;
				writer.write(" "+result);
		}
        writer.close();
